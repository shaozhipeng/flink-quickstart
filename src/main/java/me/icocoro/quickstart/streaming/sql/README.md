...
&emsp;EventTimeTrigger源码中，在onElement（会判断水位线）和onEventTime时有机会TriggerResult.FIRE。  
&emsp;前提是假定Flink处理的数据是「永不停息」的数据流时才会有『正确的实时结果』。  
&emsp;只要两个eventtime中间间隔过大，大于时间窗口间隔，或者说窗口的end时间还没到就没有新的数据了【流中断，即没有element当然也没有eventtime和WaterMark】。  
&emsp;那么最近一次的窗口输出结果一定是不及时-非实时的（如果用eventtime去做流的有界窗口聚合，必需有就近的未来数据支撑，一旦中断就不实时了）。    
&emsp;而且必需等到新的数据流接上，才会输出新的数据流之前没有及时输出的窗口结果。    
&emsp;要想『真实时TriggerResult』：
1. 使用processing-time。
